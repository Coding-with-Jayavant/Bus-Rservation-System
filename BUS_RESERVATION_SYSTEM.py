...
class travel:
    ...
    #==========================================Define Functiom==================================================
        ...
        def Reset():
            ...
            self.txtBusTax.configure(state=DISABLED)
            ...
            self.txtNONAC.configure(state=DISABLED)
        ...
        def Receiptt():
            if reset_counter is 0 and Firstname.get()!="" and Surname.get()!="" and Address.get()!="" and Postcode.get()!="" and Mobile.get()!="" and Telephone.get()!="" and Email.get()!="":
                ...
                self.txtReceipt1.insert(END,'Bus No:\n')
                self.txtReceipt2.insert(END, 'MH ' + Receipt_Ref.get() + " CQ\n")
                ...
                self.txtReceipt1.insert(END,'NONAC:\n')
                self.txtReceipt2.insert(END, NONAC.get() + "\n")
                self.txtReceipt1.insert(END,'AC:\n')
                self.txtReceipt2.insert(END, AC.get() + "\n")
        ...
        def Bus_Tax():
            global Item1
            if var1.get() == 1:
                self.txtBusTax.configure(state = NORMAL)
                Item1=float(50)
                BusTax.set("Rs " + str(Item1))
            elif var1.get() == 0:
                self.txtBusTax.configure(state=DISABLED)
                BusTax.set("0")
                Item1=0
        ...
        def Kilo():
            if var2.get() == 0:
                ...
                if varl1.get() == "yeola":
                    switch ={"vaijapur": 10,"shirdi": 8,"kopargaon":6,"yeola": 0}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "vaijapur":
                    switch ={"vaijapur": 0,"shirdi": 2,"kopargaon":25,"yeola": 10}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "shirdi":
                    switch ={"vaijapur": 2,"shirdi": 0,"kopargaon":3,"yeola": 8}
                    Km.set(switch[varl2.get()])
                elif varl1.get() == "kopargaon":
                    switch ={"vaijapur": 5,"shirdi": 3,"kopargaon":0,"yeola": 6}
                    Km.set(switch[varl2.get()])
        ...
        def selectbus():
            ...
            if busType.get() == 1:
                ...
                self.txtAC.configure(state = DISABLED)
                AC.set("0")
                self.txtNONAC.configure(state = NORMAL)
                Item5 = float(10)
                NONAC.set("Rs "+ str(Item5))
            else:
                self.txtStandard.configure(state =DISABLED)
                Standard.set("0")
                self.txtNONAC.configure(state = DISABLED)
        ...
        def Total_Paid():
            if ((var1.get() == 1 and var2.get() == 1 and var3.get() == 1 or var4.get() == 1) and busType.get() != 0 and journeyType.get() != 0 and (varl1.get() != "" and varl2.get() !="")):
   ...
   #================================================mainframe========================================================================
        ...
        self.lblTitle=Label(Tops,font=('arial',70,'bold'),text="        BUS Booking System ")
        self.lblTitle.grid()
    ...
    #================================================customerframedetail=============================================================
        ...
        CustomerName=LabelFrame(FrameDetails, width=150,height=250,bd=10, font=('arial',12,'bold'),text="Customer Detail", relief=RIDGE)
    ...
    #===============================================Bus Information==============================================================
        ...
        self.cboPickup['value']=('','yeola','kopargaon','shirdi','vaijapur')
        self.cboPickup.current(0)
        ...
        self.cboDrop['value']=('','vaijapur','shirdi','yeola','kopargaon')
        self.cboDrop.current(0)
        self.cboDrop.grid(row=1,column=1)

    #===============================================Bus Information==============================================================

        self.chkBusTax=Checkbutton(TravelFrame,text="Bus Tax(Base Charge) *",variable = var1, onvalue=1, offvalue=0,font=('arial',16,'bold'),command=Bus_Tax).grid(row=3, column=0, sticky=W)
        self.txtBusTax=Label(TravelFrame,font=('arial',14,'bold'),textvariable=BusTax,bd=6,width=18,bg="white",state= DISABLED,justify=RIGHT,relief=SUNKEN)
        self.txtBusTax.grid(row=3,column=1)
    ...
    #==========================================================Busselect=======================================================================
        ...
        self.chkStandard=Radiobutton(Book_Frame,text="Standard",value=1,variable = busType,font=('arial',14,'bold'),command=selectbus).grid(row=0, column=0, sticky=W)
        ...
        self.chkNONACd=Radiobutton(Book_Frame,text="NONAC",value=2,variable = busType,font=('arial',14,'bold'),command=selectbus).grid(row=1, column=0, sticky=W)
        self.txtNONAC= Label(Book_Frame,font=('arial',14,'bold'),width =7,textvariable=NONAC,bd=5, state= DISABLED, justify=RIGHT,bg="white",relief=SUNKEN)
        self.txtNONAC.grid(row=1,column=1)

        self.chkAC = Radiobutton(Book_Frame,text="AC",value=3,variable = busType,font=('arial',14,'bold'),command=selectbus).grid(row=2, column=0)
        ...
        self.txtAC.grid(row=2,column=1)
